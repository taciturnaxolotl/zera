...
const ICON_PATH = 'static/favicon/android-chrome-512x512.png';
...
interface RecordMapping {
	...
	iconBlobRef?: string; // CID of the uploaded icon blob
...
function parseFrontmatter(content: string): Record<string, any> {
	...
	for (const line of lines) {
		// Skip TOML table headers and blank lines
...
async function main() {
	...
	if (!mapping.publicationUri) {
		...
		if (!DRY_RUN && existsSync(ICON_PATH)) {
			const iconData = readFileSync(ICON_PATH);
			const iconHash = createHash('sha256').update(iconData).digest('hex');

			// Fetch current publication record to check existing icon
			const pubRkey = mapping.publicationUri!.split('/').pop()!;
			const currentPub = await ok(
				client.get('com.atproto.repo.getRecord', {
					params: { repo: session.did, collection: COLLECTION_PUB, rkey: pubRkey },
				}),
			);
			const currentIconRef = (currentPub.value as any)?.icon?.ref?.$link;

			if (currentIconRef !== mapping.iconBlobRef || !mapping.iconBlobRef) {
				// Icon changed or wasn't tracked — upload and update
				console.log('Icon changed or missing, uploading...');
				const blob = new Blob([iconData], { type: 'image/png' });
				const uploadResult = await ok(
					client.post('com.atproto.repo.uploadBlob', { input: blob }),
				);

				const updatedRecord = { ...(currentPub.value as any), icon: uploadResult.blob };
				await ok(
				...
				mapping.iconBlobRef = uploadResult.blob.ref.$link;
				console.log(`Updated publication icon: ${mapping.iconBlobRef}`);
			} else {
				console.log('Publication icon is up to date');
			}
		}
	}
